                ...
                <tbody id="tbody">
                    @foreach ($countries as $country)
                        <tr>
                            ...
                            <td class="py-4 px-6">{{ $country->nama_negara }}</td>
                            <td class="py-4 px-6">{{ $country->code }}</td>
                            <td class="py-4 px-6 flex flex-row gap-2 items-center justify-center">
                                <a href="{{ route('dashboard.negara.edit', ['code' => $country->code]) }}" class="py-0.5 px-2 bg-ds-yellow/20 text-ds-yellow rounded-lg cursor-pointer"><i class="fa-solid fa-pen"></i> Edit</a>
                                <button type="button" class="py-0.5 px-2 bg-ds-red/20 text-ds-red rounded-lg cursor-pointer" onclick="deleteData({{ $country->id }})">
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 text-center">
                    <form id="formDelete" action="{{ route('dashboard.negara.delete' , ['code'  => \Route::current()->parameter('code') ]) }}" method="post">
