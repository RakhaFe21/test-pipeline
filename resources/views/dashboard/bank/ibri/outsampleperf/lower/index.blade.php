    ...
    <section class="p-5">
        <div class="fle flex-col w-full mb-6">
            ...
            <div class="flex flex-row gap-2 mt-1">
                <span>Banking, Data, Out Sample Performance, Lower Threshold</span>
            </div>
        </div>
        ...
        <div class="overflow-x-auto relative shadow-md sm:rounded-lg mb-4" id="tableDiv1">
            <table class="w-full text-sm text-left text-center" id="OutSamplePerformanceTable">
                ...
                <tbody id="bodyOSPTable" class="skalz-tbody">
...
@push('scripts')
    <script>
        ...
        $('#selectVariable').on('change', function () {
            ...
            async function reqSignal(variable) {
                try {
            ...
            async function renderRangkuman(data, low) {
                data.forEach(tr => {
                    var td
                    var val
                    tr.forEach((el, index) => {
                        if (index === 0) {
                            td += '<td class="border border-slate-300 py-4 px-6"'+(low === el ? 'style="background: #00B050"' : null)+'>'+el +'</td>'
                        } else if(index > 11 && index < 15){
                            td += '<td class="border border-slate-300 py-4 px-6">'+el.toFixed(5) +'</td>'
                        }
            ...
            async function getResultMonth() {
                ...
                let resultSignal = await reqSignal(variable)
                ...
                renderRangkuman(resultFilter, lowGsb)
            }
            ...
            async function convertHorizon(data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (parseInt(data[i].tahun) > 2011 ) {
                        ...
                        data[i].nilaiMonth24_2 = data[i].nilaiMonth_24 ** 2
                        result[i] = data[i]
                    }
                }
                return result
