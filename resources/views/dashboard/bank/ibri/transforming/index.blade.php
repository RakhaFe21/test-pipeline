            ...
            <table class="w-full text-sm text-left text-center">
                ...
                <tbody id="tbody">
                    ...
                    <tr>  
                        ...
                        <td class="py-4 px-6 {{ $min_npf == $stdevs[0][$i] ? 'text-ds-red font-bold' : ''}}">{{number_format($stdevs[0][$i], 3)}}</td>  
                        <td class="py-4 px-6 ">{{number_format($means[0][$i], 3)}}</td> 
                        <td class="py-4 px-6 {{ $min_car == $stdevs[1][$i] ? 'text-ds-red font-bold' : '' }}">{{number_format($stdevs[1][$i], 3)}}</td>  
                        <td class="py-4 px-6 ">{{number_format($means[1][$i], 3)}}</td>
                        <td class="py-4 px-6 {{ $min_ipr == $stdevs[2][$i] ? 'text-ds-red font-bold' : '' }}">{{number_format($stdevs[2][$i], 3)}}</td>  
                        <td class="py-4 px-6 ">{{number_format($means[2][$i], 3)}}</td>
                        <td class="py-4 px-6 {{ $min_fdr == $stdevs[3][$i] ? 'text-ds-red font-bold' : '' }}">{{number_format($stdevs[3][$i], 3)}}</td>  
